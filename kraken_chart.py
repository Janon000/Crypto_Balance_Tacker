...
class kraken_chart():
    ...
    def get_ledger_history(self):
        ...
        ledger, count = self.k.get_ledgers_info()
        offset = 50
        while True:
            try:
                time.sleep(2)
                ledger2, count = self.k.get_ledgers_info(ofs=offset, start=unix)
    ...
    def get_asset_ticker(self, ledger):
        """
        ...
        renames = {'ETH2': 'ETH', 'ETH2.S': 'ETH', 'FLOWH': 'FLOW', 'FLOWH.S': 'FLOW', 'USD.HOLD': 'USD',
                   'USD.M': 'USD',
                   'ZUSD': 'USD'}
        exclusions = ['CHF', 'KFEE', 'ZCAD', 'ZJPY']

        # Get the list of assets available on Kraken exchange
        assets = self.k.get_asset_info()
    ...
    def get_coin_history(self, ticker_dict, currency='USD'):
        """
        ...
        for asset, altname in ticker_dict.items():
            if altname != 'USD':
                ...
                ohlc, last = self.k.get_ohlc_data(pair, interval=1440)
    ...
    def process_ledger(self, ledger):
        ...
        ticker_dict = self.get_asset_ticker(ledger)
        ...
        history = self.get_coin_history(ticker_dict)
    ...
    def chart_data(self, ledger):

        data = self.process_ledger(ledger)
        ...
        fig.update_xaxes(
            ...
            rangeselector=dict(
                buttons=list([
                    ...
                    dict(count=len(data.index), label="year", step="day", stepmode="backward"),
                    dict(step="all")
                ])
            )
        )
